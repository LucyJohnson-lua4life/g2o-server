...
local function cmd_acp(pid, params)
{
	...
	sendMessageToPlayer(pid, 0, 255, 0, "/hp id value - Set player health")
	sendMessageToPlayer(pid, 0, 255, 0, "/mana id value - Set player mana")
	sendMessageToPlayer(pid, 0, 255, 0, "/world id path - Change player world")
	sendMessageToPlayer(pid, 0, 255, 0, "/instance id name - Change player instance")
	sendMessageToPlayer(pid, 0, 255, 0, "/heal id - Heal player")
	sendMessageToPlayer(pid, 0, 255, 0, "/kill id - Kills player")
...
local function cmd_hp(pid, params)
{
	if (checkPermission(pid, LEVEL.ADMIN))
	{
		local args = sscanf("dd", params)
		if (!args)
		{
			sendMessageToPlayer(pid, 255, 0, 0, "ACP: Type /hp id value")
			return
		}
		...
		if (!isPlayerSpawned(id))
		{
			sendMessageToPlayer(pid, 255, 0, 0, "ACP: You cannot give health to unconnected or unspawned player!")
			return
		}

		if (value < 1) value = 1
		if (getPlayerMaxHealth(id) < value) setPlayerMaxHealth(id, value)
		setPlayerHealth(id, value)

		sendMessageToPlayer(pid, 0, 255, 0, format("ACP: You changed %s health to %d", getPlayerName(id), value))
		sendMessageToPlayer(id, 0, 255, 0, format("Health was changed to %d by %s", value, getPlayerName(pid)))
	}
}
...
local function cmd_mana(pid, params)
{
	if (checkPermission(pid, LEVEL.ADMIN))
	{
		...
		if (!args)
		{
			sendMessageToPlayer(pid, 255, 0, 0, "ACP: Type /mana id value")
			return
		}
		...
		if (!isPlayerSpawned(id))
		{
			sendMessageToPlayer(pid, 255, 0, 0, "ACP: You cannot give mana to unconnected or unspawned player!")
			return
		}

		if (value < 1) value = 1
		if (getPlayerMaxMana(id) < value) setPlayerMaxMana(id, value)
		setPlayerMana(id, value)

		sendMessageToPlayer(pid, 0, 255, 0, format("ACP: You changed %s mana to %d", getPlayerName(id), value))
		sendMessageToPlayer(id, 0, 255, 0, format("Mana was changed to %d by %s", value, getPlayerName(pid)))
	}
}

//---------------------------------------

local function cmd_world(pid, params)
{
	if (checkPermission(pid, LEVEL.MOD))
	{
		local args = sscanf("ds", params)
		if (!args)
		{
			sendMessageToPlayer(pid, 255, 0, 0, "ACP: Type /world id path")
			return
		}
		...
		if (!isPlayerSpawned(id))
		{
			sendMessageToPlayer(pid, 255, 0, 0, "ACP: You cannot change world for unconnected or unspawned player!")
			return
		}

		setPlayerWorld(id, path)

		sendMessageToPlayer(pid, 0, 255, 0, format("ACP: You changed %s world to %s", getPlayerName(id), path))
		sendMessageToPlayer(id, 0, 255, 0, format("Changing world to %s by %s", path, getPlayerName(pid)))
	}
}
...
local function cmd_instance(pid, params)
{
	if (checkPermission(pid, LEVEL.MOD))
	{
		...
		if (!args)
		{
			sendMessageToPlayer(pid, 255, 0, 0, "ACP: Type /instance id name")
			return
		}
		...
		if (!isPlayerSpawned(id))
		{
			sendMessageToPlayer(pid, 255, 0, 0, "ACP: You cannot change instance of unconnected or unspawned player!")
			return
		}

		setPlayerInstance(id, instance)

		sendMessageToPlayer(pid, 0, 255, 0, format("ACP: You changed %s instance to %s", getPlayerName(id), instance))
		sendMessageToPlayer(id, 0, 255, 0, format("Changing instance to %s by %s", instance, getPlayerName(pid)))
	}
}
...
local function cmd_kill(pid, params)
{
	if (checkPermission(pid, LEVEL.MOD))
	{
		...
		if (!args)
		{
			sendMessageToPlayer(pid, 255, 0, 0, "ACP: Type /kill id")
			return
		}
		...
		if (!isPlayerSpawned(id))
		{
			sendMessageToPlayer(pid, 255, 0, 0, "ACP: You cannot kill unconnected or unspawned player!")
			return
		}

		setPlayerHealth(id, 0)

		sendMessageToPlayer(pid, 0, 255, 0, format("ACP: You killed %s", getPlayerName(id)))
		sendMessageToPlayer(id, 0, 255, 0, format("You were killed by %s", getPlayerName(pid)))
	}
}
...
local function playerJoin(pid)
{
	...
	local mac = getPlayerMacAddr(pid)
	local serial = getPlayerSerial(pid)
	local uid = getPlayerUID(pid)

	foreach (credentials in MOD_CREDENTIALS)
	{
		if ("ip" in credentials && credentials.ip != ip)
			continue

		if ("mac" in credentials && credentials.mac != mac)
			continue

		if ("serial" in credentials && credentials.serial != serial)
			continue

		if ("uid" in credentials && credentials.uid != uid)
			continue

		Player[pid].rank = LEVEL.MOD
	}

	foreach (credentials in ADMIN_CREDENTIALS)
	{
		if ("ip" in credentials && credentials.ip != ip)
			continue
